...
def _plugin_readme() -> str:
    ...
    return (
        ...
        "`/refine` command. To trigger refinement, invoke the bundled "
        "`skill-refiner` skill manually at end of session — it reads "
        "`.metplot/task-log.jsonl` and writes drafts to "
        "`.metplot/refinements/` for human review.\n"
        "- **No Stop hook.** Codex exposes no Stop-hook surface, so "
        "the refiner is manual-trigger only on this host. (Claude "
        "Code fires the hook automatically at session end; other "
        "hosts wait for native hook support.)\n"
    )
